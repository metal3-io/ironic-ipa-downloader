...
if [ -e $FFILENAME.headers ] ; then
    ...
    cd $TMPDIR
    curl -g --verbose --dump-header $FFILENAME.headers -O https://images.rdoproject.org/train/rdo_trunk/$SNAP/$FFILENAME --header "If-None-Match: $ETAG"
    ...
    cd $TMPDIR
    curl -g --verbose --dump-header $FFILENAME.headers -O https://images.rdoproject.org/train/rdo_trunk/$SNAP/$FFILENAME
fi
